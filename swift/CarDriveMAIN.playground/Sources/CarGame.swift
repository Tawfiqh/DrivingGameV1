...
public class CarGame {
  ...
  static let xAxisRange: Double = 20.0
  var roadWidth: Double = Double(CarGame.xAxisRange) / 2.0
  static let startY: Double = 10.0
  ...
  func setupKeyboardControls() {
    print("TBC - implement setupKeyboardControls")
  ...
  func setupGameRunLoop() {
    print("TBC - setupGameRunLoop")
    ...
    func gameRunLoop(runLoopIntervalMilliseconds: number): void {
    ...
    func updatePlayer(runLoopIntervalMilliseconds: number): void {
      this.gameState.player.updatePosition(runLoopIntervalMilliseconds / 1000);
  }

    func updateMap(runLoopIntervalMilliseconds: number = 0): void {
    ...
    func checkCollisionsInList(player: Player, objectsToCheck: EnvironmentObject[]): boolean {
